...
下文所有命令均在仓库根目录执行。游戏代码位于 `outputs/community-seasons/`，所有测试和 QA 测试页面都直接使用这份源码。

操作方式、存档和游戏规则请参阅[游戏说明](outputs/community-seasons/README.zh-CN.md)和[玩法指南](outputs/community-seasons/GAMEPLAY.zh-CN.md)。
...
构建过程会同步题库、重新生成第三方许可声明、检查类型，并将产物写入 `outputs/community-seasons/dist/`。使用以下命令在本地预览构建结果：

```sh
npm --prefix outputs/community-seasons run preview -- --port 4173
```

打开 [http://127.0.0.1:4173](http://127.0.0.1:4173)。只有 `outputs/community-seasons/dist/` 是正式发布产物；QA 测试页面、调试控制和 QA URL 参数开关不得进入发布版本。
...
| 路径 | 内容 |
| --- | --- |
| `outputs/community-seasons/` | 游戏源码、翻译、题库、资源和构建配置 |
...
`outputs/` 和 `work/` 是源码目录，不能当作临时构建产物删除。保留这些名称是为了维持测试中的相对导入路径。构建产物、已安装的依赖和 QA 结果均由 Git 忽略。
