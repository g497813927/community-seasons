...
async function fixture(t) {
  ...
  const dist = path.join(root, 'qa/preview/dist');
  await fs.mkdir(path.join(root, 'src'), { recursive: true });
  await fs.mkdir(dist, { recursive: true });
  await fs.writeFile(path.join(root, 'src/game.ts'), 'export const score = 1;');
  await fs.writeFile(path.join(root, 'qa/preview/bootstrap.ts'), 'import "../../src/game";');
...
test('build provenance accepts its source snapshot and ignores generated files', async t => {
  ...
  await fs.mkdir(path.join(root, 'qa/preview/.vite-cache'), { recursive: true });
  await fs.writeFile(path.join(root, 'qa/preview/.vite-cache/cache.json'), '{}');
...
test('a missing or malformed record and edits during compilation cannot pass provenance checks', async t => {
  ...
  const plugin = previewBuildInfo(root);
  await plugin.buildStart.call(context);
  await fs.writeFile(path.join(root, 'qa/preview/bootstrap.ts'), 'import "../../src/other";');
