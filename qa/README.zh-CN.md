...
构建会将游戏、预览及依赖锁文件的哈希写入 `qa-build-info.json`。浏览器检查会在启动前拒绝缺失或过期的构建记录；修改源码或依赖后，请重新运行 `npm run qa:build`。编译过程中源码发生变化也会使构建失败，确保报告对应实际生成被测预览的源码。
...
Android 使用 `--platform android` 和对应检查器地址。桥接支持时还可执行 `screenshot`。由于选定地址会写入报告，预览地址不得含有查询参数、凭据或片段标识。发现命令只返回地址完全匹配的目标；每次选定页面的操作都会验证精确框架、QA 标识、独立存储前缀和已禁用的云服务，拒绝缺失或不唯一的目标。命令不会导航、重置游戏进度或操作其他标签页。测量只重置诊断数据；页面不可见、失焦或缺少真实用户激活时停止。设备报告保留实际浏览器标识和所选平台；声称真机验证时，应同时保存硬件证据。
