...
const config=JSON.parse(fs.readFileSync('qa/archive/phone-cart-fix-qa/preview.json','utf8'));
...
try{
 ...
 if(action==='export'){
  ...
  fs.writeFileSync('qa/archive/phone-cart-fix-qa/physical-result.json',result+'\n');result='Saved scoped physical-result.json';
 }
 if(action==='screenshot'){
  const r=await call('Page.captureScreenshot',{format:'png'});if(typeof r.data!=='string')throw Error('Screenshot unavailable');fs.writeFileSync('qa/archive/phone-cart-fix-qa/physical-screenshot.png',Buffer.from(r.data,'base64'));result='Saved physical-screenshot.png';
 }
 console.log(result);
