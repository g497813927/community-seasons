...
const engine = await import("./compiled/engine.mjs");
const { createBoostLevels } = await import("./compiled/boosts.mjs");
const {
...
function persistFailure(session, error) {
  ...
  const payload = {
    ...
    replay: `ENGINE_FUZZ_REPLAY=work/community-tests/engine-fuzz-failures-20260910/${filename} node --test work/community-tests/engine-state-fuzz.test.mjs`,
  };
