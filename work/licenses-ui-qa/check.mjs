...
for(const [width,height]of[[320,568],[390,844],[1280,900]])for(const locale of['en','zh-CN'])for(const scale of[1,2]){
 ...
 try{
  ...
  await p.locator('.licenses-launcher').click();await p.locator('.license-entry').first().waitFor();assert.equal(count(requests),1);assert.equal(await p.locator('.license-entry').count(),inventory.packages.length);assert.equal(await p.locator('.licenses-title').evaluate(el=>document.activeElement===el),true);await scaleText(p,scale);checkLayout(await layout(p));await p.locator('.licenses-dialog').evaluate(el=>el.scrollTop=0);await p.screenshot({path:`work/licenses-ui-qa/${width}-${locale}-${scale}-initial.png`});
 ...
 }catch(e){Object.assign(row,{passed:false,error:String(e),errors});await p.screenshot({path:`work/licenses-ui-qa/${width}-${locale}-${scale}-failure.png`}).catch(()=>{});}
