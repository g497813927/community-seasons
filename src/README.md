...
Your outfit appears during loading, on the home screen, while running and riding the cart, and after retries. All outfits are cosmetic: hats, shoes and effects do not change speed, collisions, rewards, skills or boosts. Owned skins and accessories and your selected outfit save in this browser and sync through Toy when cloud sync is enabled.
...
The game automatically reduces decorative scenery, paving detail and canvas resolution under sustained load. Rendering uses paced 60/30/20/15/10 FPS tiers for a steadier cadence; browsers exposing 90/120 Hz callbacks can unlock higher rates after sustained rendering headroom. Detail and frame rate recover cautiously to avoid rapid switching. A higher tier that failed a rendering check is retried only after measured capacity improves or the available display cadence changes. Text overlays and controls retain their display resolution, and input handling and the simulation clock continue independently of the rendering cap.

A preparation screen covers initial rendering calibration. A lightweight TV runner faces right in a three-quarter view and walks in place with contact shadows that follow each footfall on the loading screen, using the same proportions and equipped outfit as the game. The TV walks clockwise along the inner circular path, with the scenery moving right-to-left beneath it. Each season fills a 180-degree half of the wheel, with only two neighboring seasons mounted at once. The lower half stays hidden; after each half-turn, its old season is replaced before rotation resumes, revealing the connections without loading all four landscapes. Tap or use the keyboard to turn quickly to the next season. After calibration, the wheel turns to your current saved season before the game appears. Reduced-motion preferences keep the character still and use instant, manual season changes. This is a playful wait screen, not a download percentage. Rendering warms up behind it, quickly checks higher quality when there is headroom, and reveals the game only after the best sustainable tier has settled. It does not interrupt later gameplay. Browser callback limits before an embedded preview's first interaction do not reduce quality by themselves; actual rendering cost still counts.
