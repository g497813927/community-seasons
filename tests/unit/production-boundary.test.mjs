...
const { createProgress, readProgress, buyBooster, activateOwnedBooster } =
  await import("../helpers/compiled/store.mjs");
const { isSceneKind } = await import("../helpers/compiled/scenes.mjs");
...
test("production excludes sibling tests without rejecting a checkout beneath a tests directory", () => {
  for (const project of ["/some/project", "/tmp/tests/community-seasons", "C:\\tests\\community-seasons"]) {
    const guard = boundary.productionBoundary(`${project}/src`);
    for (const file of ["unit/fixture.mjs", "helpers/compiled/engine.mjs", "qa/preview/bootstrap.ts"]) {
      assert.throws(
        () => guard.transform("export const fixture = true", `${project}/tests/${file}`),
        /Test workspace/,
      );
    }
    for (const file of ["src/lib/game/scenes.ts", "node_modules/some-runtime/index.js"]) {
      assert.doesNotThrow(() => guard.transform("export const value = true", `${project}/${file}`));
    }
  }
});
