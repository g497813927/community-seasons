# 跨平台 QA

[English](README.md) | [简体中文](README.zh-CN.md)

使用 Node 24，在仓库根目录执行命令。通用预览直接导入当前 `src/` 游戏源码，不加载 Toy SDK，也不访问云存储；游戏启动前，浏览器存储操作已映射到 `qa-community-seasons-v1:` 前缀。生产游戏的存储键保持不变。

## 构建与检查

```sh
npm run setup
npx playwright install chromium webkit
npm run build
npm run qa:build
npm run qa:skins
npm run qa:test
npm run qa:all
```

构建会将游戏、预览及依赖锁文件的哈希写入 `qa-build-info.json`，并将相同记录嵌入预览 HTML。页面加载时，探针会在 `build` 中保存不可变副本；之后获取较新的清单不会将旧页面标记为新构建。浏览器检查会在启动前拒绝缺失或过期的构建记录；修改源码或依赖后，请重新运行 `npm run qa:build`。编译过程中源码发生变化也会使构建失败，确保报告对应实际生成被测预览的源码。

可分别使用 `qa:web`、`qa:android` 或 `qa:ios`。网页检查使用启用触摸的桌面 Chromium，Android 使用 Chromium 触摸模拟，iOS 使用 WebKit 触摸模拟。每组覆盖英文和简体中文，使用全新浏览器上下文、独立本地服务器，并阻止外部请求。这些是模拟检查，不是真机性能测量。运行器会记录并修正已复现的 WebKit 模拟矛盾：竖屏却报告 90 度方向角；初始化后 WebKit 更换方向对象时，修正仍会生效。正常横屏方向值保持不变。WebKit 滑动使用明确标记的合成触摸 PointerEvent，经过真实游戏处理器；原生滑动、旋转及性能仍需真机检查。报告和截图保存到 `results/qa/` 下带时间戳的目录。

六种平台/语言组合都逐项检查 14 种输入：完整方向键、WASD、Space、四向滑动和双击。换道要求目标车道与实际横向位置同时变化；跳跃要求新跳跃及正高度；滑行要求处于滑行动作且没有跳跃。双击必须消耗已充满的技能充能、激活护盾并锁定再充能；第一次点击单独不能激活技能。Chromium 滑动使用 CDP 触摸事件，所有平台的双击都使用 Playwright 触屏点击。

每次输入前，preview 专用观测器会准备中性动作和满充能护盾，保持路线、距离和时间。全新上下文只在隔离 QA 存档键中预置已解锁的护盾，之后经过真实开局设置。preview 通过别名接入已有的游戏工具注册入口，读取当前运行状态，不替换输入处理器或引擎动作。每项输入先启用捕获阶段的事件拦截，要求确实收到事件、游戏帧仍推进、动作没有变化，且同一结果断言失败；随后解除拦截并要求动作通过。`rows[].inputs` 保留两次试验、前后状态、首次点击状态、事件发送方式及预期失败信息。发送错误或前置条件不满足会使场景失败，不计为负向对照通过。

浏览器冒烟检查会在加载页面前安装 Playwright 时钟，并在开始游戏前暂停该时钟。键盘和触摸事件仍交给真实游戏处理；测试只按明确、有限的时间步长推进动画回调。暂停检查会推进 300ms 并要求距离不变，继续后再推进 250ms 并要求距离增加。Android 英文用例还会在输入序列中等待六秒真实主机时间，确认这个延迟不会让角色移动或结束游戏，避免较慢的 CI 在检查暂停前已撞上随机障碍。报告会将计时和帧数据标为模拟结果，不用于性能测量。交互预览与真机检查工具继续使用正常时钟。

启动检查会等待游戏的语言初始化逻辑更新 `html.lang`；报告记录文档最初的语言以及浏览器请求的语言。开始游戏前，时钟会停在读取到的页面时间，不会向前跳跃并触发尚未到期的计时器。回归测试使用已安装的 Playwright 时钟验证这一转换，包含控制消息延迟的情况。

关闭许可弹窗后，检查会等待弹窗与背景遮罩消失、文档滚动样式恢复原值，并确认焦点回到入口按钮。由于 WebKit 的原生布局与滚动操作在 CI 主机 CPU 繁忙时可能超过 10 秒，其单次操作上限为 30 秒、单个场景上限为 120 秒；Chromium 仍为 10 秒和 60 秒。报告记录这些限制与主机端操作耗时，供排查运行器问题，不用于衡量游戏性能。

运行 `npm run qa:responsive` 执行有界的首页布局回归矩阵，也可追加 `-- --engine chromium` 或 `-- --engine webkit` 选择单个引擎；使用 `-- --profile cloud` 只运行云状态组件场景，或用 `-- --profile ID` 选择报告中的单个场景。覆盖中英文的 320×568、390×664、390×844、412×915、768×1024、1024×768、1280×720、1440×900，以及 779/780/781 像素高度断点、受限 iframe、视口高度从 844px 改为 664px 再恢复、模拟云同步错误和 200% 大字场景。标题不能形成独立滚动区，文档不能横向溢出，最后一个可见游戏区域或页脚之后不能出现多余空白。小屏幕隐藏底部指南和许可入口，通过顶部按钮打开帮助、指南和许可；较大屏幕保留页脚。所有可见首页操作在滚动后必须可达，大字场景同样适用。云状态测试渲染真实生产组件，使用不联网的重试计数器和真实 UA 判断函数。带 BiliApp 标记的移动场景检查错误通知自动出现、关闭、Enter/Escape、仅一次重试、重复失败与恢复；外部移动浏览器检查仅在云功能失败或不支持时隐藏控件并建议使用哔哩哔哩，而检查中或成功同步时仍保留控件。推荐使用不含云存档术语的简单提示，并用箭头指向宿主页面右上角的打开按钮；“不再提示”偏好在隔离 QA 命名空间中跨刷新保留，不影响哔哩哔哩 App 内真实的云错误控件。桌面错误继续内联显示。任何场景都不访问 Toy，并保留通知展开及关闭后的截图。文字缩放前先快照字体大小与行高，包括新创建的通知 portal。大字场景关闭字体过渡动画，并在通知状态切换后验证计算字号确实为 200%；它用于布局压力测试，不代表某款手机的原生无障碍实现。报告和截图保存在 `results/qa/responsive-<timestamp>/`；与冒烟测试一样，运行器验证预览构建及源码哈希。

执行 `npm run qa:build` 后，运行 `npm run qa:skins`，可在桌面 Chromium、Android Chromium 模拟和 iOS WebKit 模拟中检查中英文皮肤商店。检查覆盖键盘与触摸购买、免费切换已拥有皮肤、金币不足、刷新后保留拥有情况与当前选择、暂停当局时换装，以及放大文字布局。运行器使用隔离的 QA 存档并禁用云访问，截图和结果保存在 `results/qa/skins-<timestamp>/`。这些检查验证浏览器功能与布局，不代表真机性能。

可选的全量装扮矩阵使用独立构建：先运行 `npm run qa:outfit:build`，再用 `npm run qa:outfits -- --engine all --workers 2 --limit 4` 进行明确标为部分覆盖的试跑。去掉 `--limit` 后，会覆盖 320 种机身、帽子、鞋子、特效组合 × 四季场景 × Chromium/WebKit，共 2,560 个案例。每个案例先使用当前源码渲染器运行至少 **60 秒真实时间**，随后在真实游戏页面检查传送过程、火车全部答对并领取一次奖励后返回、答错及解析，以及不按“出发”而等待原始倒计时自动判题。答案通过真实答案按钮和“出发”按钮提交；仅 QA 初始化会把角色放到车站或传送门附近。编排的渲染场景和真实界面流程分别记录，不使用加速时钟。全量测试有明确上限，但耗时较长；应根据试跑记录的帧率选择并发量，不要放宽帧检查。

结果保存在 `results/qa/outfit-matrix-<timestamp>/`，包含源码、构建和运行器哈希、逐例截图、尝试记录与断点。用 `npm run qa:outfits -- --resume 结果目录绝对路径 --workers 2` 续跑；源码、构建或运行器哈希变化时拒绝续跑，失败或中断的案例会重新执行。矩阵使用全新浏览器上下文、专用本地服务、外部网络拦截，以及独立的 `qa-community-seasons-outfit-functional-v1:` 存档命名空间，并禁用 Toy。此测试产物不能作为正式游戏上传。桌面矩阵结果不代表真机性能。

## 交互预览与设备检查

```sh
npm run qa:preview                       # 构建预览：http://localhost:4175/
npm run qa:preview -- --host 0.0.0.0     # 明确向局域网测试手机开放
npm run qa:serve                         # 开发时使用，支持热更新
```

普通手动设备检查时，用手机普通浏览器打开指定 QA 预览，保持设备解锁，并先真实触摸游戏，再测量。QA 菜单可导出帧间隔及错误诊断，不包含存档内容。帧数和最大间隔统计重置后的全部可见帧间隔，包括等待第一次回调的时间，内存占用固定。P95 使用 1 毫秒直方图区间的保守上界；超过 60 秒的间隔共用溢出区间，并以实际最大值作为上界。报告会明确说明这一近似方法。性能观察应使用构建后的预览；热更新或桌面模拟结果不能证明真机性能。

请求用户手动输入前，先识别并说明 Safari 是否由 WebDriver 控制。[Glass Pane（玻璃遮罩）](https://developer.apple.com/documentation/webkit/about-webdriver-for-safari#Glass-Panes)会拦截手动输入及窗口大小调整；打破遮罩会中断会话并永久断开自动化连接。应通过 WebDriver 操作，不得要求用户点击开始、触摸、调整窗口大小或打破遮罩。真实触摸要求与 `qa:device measure` 焦点/激活校验适用于普通手动预览，不得绕过。当前安装的 iOS CDP 桥接通过 JavaScript 事件实现 `Input.dispatchMouseEvent`，须标为脚本功能检查，不能证明手动输入或原生输入性能。

Android 需启用 USB 调试、授权本机并打开 Chrome，然后执行 `adb forward tcp:9222 localabstract:chrome_devtools_remote`。iOS 需启用 Safari 网页检查器，并运行本地 Web Inspector 到 CDP 的桥接，例如 `pymobiledevice3 webinspector cdp --host 127.0.0.1 --port 9223`。这些设备工具是外部前置条件，不是游戏依赖。参见 [iOS 桥接工具上游指南](https://github.com/doronz88/pymobiledevice3/blob/master/docs/guides/webview-debugging.md)。

```sh
# 将页面地址替换为手机中实际打开的完整预览地址。
npm run qa:device -- --platform ios --endpoint http://127.0.0.1:9223 --page http://YOUR_LAN_IP:4175/ --action list
npm run qa:device -- --platform ios --endpoint http://127.0.0.1:9223 --page http://YOUR_LAN_IP:4175/ --target TARGET_ID --action status
npm run qa:device -- --platform ios --endpoint http://127.0.0.1:9223 --page http://YOUR_LAN_IP:4175/ --target TARGET_ID --action measure --seconds 10
```

Android 使用 `--platform android` 和对应检查器地址。桥接支持时还可执行 `screenshot`。由于选定地址会写入报告，预览地址不得含有查询参数、凭据或片段标识。发现命令只返回地址完全匹配的目标；每次选定页面的操作都会验证精确框架、QA 标识、独立存储前缀和已禁用的云服务，拒绝缺失或不唯一的目标。命令不会导航、重置游戏进度或操作其他标签页。测量会先验证页面嵌入的构建哈希与当前工作区一致，再重置诊断数据。热更新页面和过期构建会被拒绝；采集期间持续检查页面构建身份，保存前再次检查本地源码。修改后须重新构建并刷新选定页面。状态和截图仍可使用，但会明确标记未构建或过期的来源。页面不可见、失焦或缺少真实用户激活时，测量也会停止。页面不可见、pagehide 和 blur 事件会持续保留到重置诊断数据；即使在两次轮询之间短暂离开并返回，也会使本次采集无效。设备报告保留实际浏览器标识和所选平台；声称真机验证时，应同时保存硬件证据。

同一份 `tests/qa/preview/dist/` 使用相对资源路径，也可部署到已获授权的独立托管预览。不得用它替换生产 Toy 或 `src/dist/`。托管预览须保留访问保护，选定的完整地址只在本地保存。设备检查器支持 Toy 预览外层页面，但拒绝生产 Toy 地址。

## 目录与维护

| 路径 | 用途 |
| --- | --- |
| `preview/` | 引用当前源码的通用预览、独立存档、禁用 Toy SDK、有容量限制的诊断 |
| `web/` | 英文/中文的桌面及触摸浏览器冒烟检查 |
| `devices/` | 明确选择 Android/iOS 目标、受校验的 CDP 操作及协议测试 |
| `tests/` | 存档隔离及预览产物回归测试 |
| `archive/` | 保留为可运行参考的历史专项测试场景 |

生产构建会拒绝导入 `tests/qa/` 或包含 QA 标识字符串。新增测试控制应放在这里，不要加入 `src/`。修改后运行 `npm run build`、`npm test`、`npm run qa:build`、`npm run qa:test` 及相关浏览器组合。游戏逻辑与模糊测试参见 [QA 指南](../../docs/QA.zh-CN.md)。

归档场景保留原行为及独立来源。许可、Android 大字和道具控制预览会在游戏启动前验证完整本地来源，仅重置各自的 QA 存储前缀，保留现有玩家及其他应用的数据。带日期的 iPhone 云端测试也可能使用其隔离的 Toy 测试键。运行前请阅读代码及说明。旧测量日期与特定设备假设只是历史记录，不代表本次验证结果。

```sh
npm run qa:archive:phone:build
npm run qa:archive:iphone:build
node --test tests/qa/archive/phone-cart-fix-qa/*.test.mjs tests/qa/archive/iphone-qa/*.test.mjs
npm run qa:archive:licenses:serve         # 在另一个终端中运行
npm run qa:archive:licenses
npm run qa:archive:android:serve          # 在另一个终端中运行
npm run qa:archive:android
```
