...
try {
  ...
  for (const config of selectedCases) {
    ...
    try {
      ...
      await page.screenshot({ path: `tests/qa/archive/boost-controls-qa/${width}-${locale}-${scale}-running.png` });
      ...
      await page.screenshot({ path: `tests/qa/archive/boost-controls-qa/${width}-${locale}-${scale}-failure.png` }).catch(() => {});
    }
    rows.push(row);
  ...
  fs.writeFileSync('tests/qa/archive/boost-controls-qa/results.json', JSON.stringify({
