  ...
  test(name, { skip: !!process.env.FC_SAVE_CASE && process.env.FC_SAVE_CASE !== name }, () => {
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          ...
          replay: `FC_SAVE_CASE='${name}' FC_SAVE_SEED=${seed}${replayPath ? ` FC_SAVE_PATH='${quotedPath}'` : ""} node --test tests/property/save-invalid.test.mjs`,
        }, null, 2),
      );
      writeResults();
      throw error;
    ...
    if (details.failed) {
      const artifact = {
        ...
        replay: `FC_SAVE_CASE='${name}' FC_SAVE_SEED=${details.seed} FC_SAVE_PATH='${details.counterexamplePath}' node --test tests/property/save-invalid.test.mjs`,
      };
      fs.writeFileSync(
        new URL(`./save-invalid-failure-${name}.json`, import.meta.url),
