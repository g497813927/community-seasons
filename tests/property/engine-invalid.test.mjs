...
const { boostDefinition } = await import("../helpers/compiled/boosts.mjs");
const { readProgress, createProgress, activateOwnedBooster, activatePermanentSkill } = await import("../helpers/compiled/store.mjs");
  ...
  test(description, { skip: Boolean(process.env.FC_PROPERTY && process.env.FC_PROPERTY !== id) }, () => {
    ...
    try {
      ...
      const report = {
        ...
        replay: `FC_PROPERTY=${id} FC_SEED=${seed}${replayPath ? ` FC_PATH='${quotedPath}'` : ""} node --test tests/property/engine-invalid.test.mjs`,
      };
      reports.push(report);
    ...
    const report = {
      ...
      replay: `FC_PROPERTY=${id} FC_SEED=${result.seed}${result.counterexamplePath ? ` FC_PATH=${result.counterexamplePath}` : ""} node --test tests/property/engine-invalid.test.mjs`,
    };
    reports.push(report);
