...
The terminal prints the protected deployment URL, expiry and a local `access.json` path under `results/qa/vercel-*/`. Before requesting a share secret, the script creates a unique directory and an exclusive empty file, then verifies their POSIX permissions (`0700` and `0600`). It rechecks privacy before writing through the same open file handle; failed attempts clean up the newly created file without overwriting an existing file. The directory is Git-ignored. Open the file locally and use the link on the test device. Treat the full URL as a password, and delete the file when testing ends.

Private link creation requires a filesystem that enforces POSIX permissions, such as a local macOS/Linux filesystem. Windows sharing is rejected because [Node's mode checks cannot establish Windows ACL privacy](https://nodejs.org/api/fs.html#file-modes). Filesystems that ignore the required modes are rejected before minting; reported modes cannot prove privacy on storage with different access semantics. `qa:deploy -- --deploy-only` remains available without private-file support: use Vercel Authentication, or run `qa:share` on a supported filesystem. A failure after requesting a link may leave that remote link active until expiry; use the revocation controls below if needed.
