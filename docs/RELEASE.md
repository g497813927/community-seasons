...
On 2026-09-17, an authorized cover-and-icon-only update replaced the cover with a 1200 × 900 PNG whose meaningful content fits a centered 1200 × 675 safe area. The cover includes lower-left decorative ripples. The new 500 × 500 PNG icon and cover mark use the same Flower2 geometry as the game header. No game package was uploaded. The CLI initially returned `auditing`; the subsequent listing reported `published`, new poster/icon URLs, and `PASSWORD` visibility. Visibility and password flags were omitted to preserve the existing password. Before submission, both language covers passed glyph-bound checks and visual crop review, the icon passed small-size/circular-mask review, all 330 regression tests passed, and the production build succeeded. Reproducible assets and export instructions are in [the thumbnail generator guide](../scripts/readme-thumbnail/README.md); local PNGs and review reports are under `results/readme-thumbnail/` (Git-ignored).
